...
function renderStories(){
    ...
    for(let i = 0; i < stories.length; i++){
        ...
        post.append(`<div class='detail-story'>` + stories[i][0] + ", " + stories[i][1] + ", " + stories[i][2] + `</div>`);
        let textBlock = $("<div id='block-story'>" + stories[i][3] + "</div>")
        textBlock.append(`<div class='text-story'>` + stories[i][4] + `</div>`);
...
function checkTag(story){
    let selected = $('#type-select').val().toLowerCase();
    let tags = story[4].toLowerCase();
    if(selected == "all"){
        return true;
    }
